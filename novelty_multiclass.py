"""
main file for experiments with pure training set for multi-class datasets
"""
                        ############# IMPORTS #################
...
for i in all_classes:
    ...
    if dataset == 'STL':
        # STL is a smaller dataset, so we use the anomalies discarded from the train set too
    ...
    if dataset in ['MNIST','FMNIST']:
        # noemalize over all pixels rather than by column
    ...
    test_x = test_x.to(device)
              ...
              ############## MODEL LOADING ##################    
    if load_model == True:
        path = "saved_models/%s/%s/%s/%d" %(dataset,data_type,normality_type,digit)
        ...
        torch.save({
                    ...
                    'hidden_size': [enc_hidden,dec_hidden],
    ...
    density_type = 'NF'
    n_neighbors = 100
    nb = neighbours.NearestNeighbors(algorithm = 'brute', n_neighbors = 40)
    nb.fit(train_enc)
    nb_distance, nb_idx = nb.kneighbors(test_enc, n_neighbors = 40)
    
    if density_type == 'KNN': #n_neighbors = 5,20,40
        local_density_score = np.mean(nb_distance[:,:n_neighbors],axis=1)
    
    if density_type == 'GD': #n_neighbors = 100
    ...
    if density_type == 'NF': #n_neighbours = 100
        local_density_score = maf.nf(train_enc, test_enc).cpu().numpy()
        local_density_score = -1e-2*local_density_score

    # LOCAL RECONSTRUCTION SCORE
    nb_reconstruction = np.empty((len(test_enc),40))
