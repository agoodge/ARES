...
parser.add_argument('--evaluate', default = False, help='Evaluate a flow.')
#parser.add_argument('--restore_file', type=str, help='Path to model to restore.')
#parser.add_argument('--generate', action='store_true', help='Generate samples from a model.')
#parser.add_argument('--data_dir', default='./data/', help='Location of datasets.')
#parser.add_argument('--output_dir', default='./results/maf/MNIST/')
...
# model
parser.add_argument('--model', default='mademog', help='Which model to use: made, maf.')
# made parameters
...
parser.add_argument('--n_components', type=int, default=2, help='Number of Gaussian clusters for mixture of gaussians models.')
...
# training params
parser.add_argument('--batch_size', type=int, default=var.batch_size)
...
def train_and_evaluate(model, train_loader, test_loader, optimizer, args):
    ...
    eval_logprob = evaluate(model, test_loader, i, args)
    
    return eval_logprob

    torch.save(eval_logprob, os.path.join(args.output_dir, 'logprobs.pt'))
...
def nf(train_enc, test_enc):
    ...
    train_dataloader = torch.utils.data.DataLoader(torch.tensor(train_enc), batch_size = args.batch_size)
    test_dataloader = torch.utils.data.DataLoader(torch.tensor(test_enc), batch_size = args.batch_size)
    
    args.input_size = train_dataloader.dataset.shape[1]
    args.input_dims = train_dataloader.dataset.shape[1]
    args.cond_label_size = None #train_dataloader.dataset.label_size if args.conditional else None
    ...
    if args.train:
        eval_logprob = train_and_evaluate(model, train_dataloader, test_dataloader, optimizer, args)
